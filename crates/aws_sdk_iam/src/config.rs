...
impl Builder {
    ...
    /// impl aws_sdk_iam::config::auth::ResolveAuthScheme for CustomAuthSchemeResolver {
    ///     fn resolve_auth_scheme<'a>(
    ///         &'a self,
    ///         _params: &'a aws_sdk_iam::config::auth::Params,
    ///         _cfg: &'a ConfigBag,
    ///         _runtime_components: &'a RuntimeComponents,
    ...
    ///
    /// ```no_run
    /// use aws_sdk_iam::config::{AsyncSleep, Config, SharedAsyncSleep, Sleep};
    ///
    /// #[derive(Debug)]
    ...
    ///
    /// ```no_run
    /// use aws_sdk_iam::config::{AsyncSleep, Builder, Config, SharedAsyncSleep, Sleep};
    ///
    /// #[derive(Debug)]
    /// pub struct ForeverSleep;
    ///
    ...
    /// # Default Behavior
    ///
    /// When no retry partition is explicitly set, the SDK automatically creates a default retry partition named `iam`
    /// (or `iam-<region>` if a region is configured).
    /// All IAM clients without an explicit retry partition will share this default partition.
    ///
    /// # Notes
    ///
    ...
    /// ```no_run
    /// use aws_sdk_iam::config::Config;
    /// use aws_sdk_iam::config::retry::{RetryPartition, TokenBucket};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_iam::config::Config;
    /// use aws_sdk_iam::config::retry::{ClientRateLimiter, RetryConfig, RetryPartition};
    ///
    ...
    /// #    fn meta(&self) -> &aws_sdk_iam::error::ErrorMetadata { todo!() }
    /// # }
    ///
    ...
    pub fn build(mut self) -> Config {
        ...
        layer.store_put(::aws_types::SigningName::from_static("iam"));
        layer
        ...
        Config {
            config: crate::config::Layer::from(layer.clone())
                .with_name("aws_sdk_iam::config::Config")
                .freeze(),
            cloneable: layer,
...
impl ServiceRuntimePlugin {
    pub fn new(_service_config: crate::config::Config) -> Self {
        let config = {
            let mut cfg = ::aws_smithy_types::config_bag::Layer::new("AWSIdentityManagementV20100508");
    ...
    pub(crate) fn new(
        ...
        Self {
            config: ::aws_smithy_types::config_bag::Layer::from(layer)
                .with_name("aws_sdk_iam::config::ConfigOverrideRuntimePlugin")
                .freeze(),
            components,
        }
    }
