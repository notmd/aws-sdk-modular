...
/// Configuration for a aws_sdk_sesv2 service client.
///
...
impl Builder {
    ...
    /// use aws_sdk_sesv2::config::Config;
    ...
    /// # fn example() {
    /// use std::time::Duration;
    /// use aws_sdk_sesv2::config::{Builder, Config};
    /// use aws_smithy_runtime::client::http::hyper_014::HyperClientBuilder;
    ///
    ...
    /// }
    ///
    /// let mut builder = aws_sdk_sesv2::Config::builder();
    ...
    /// impl aws_sdk_sesv2::config::auth::ResolveAuthScheme for CustomAuthSchemeResolver {
    ///     fn resolve_auth_scheme<'a>(
    ///         &'a self,
    ///         _params: &'a aws_sdk_sesv2::config::auth::Params,
    ///         _cfg: &'a ConfigBag,
    ///         _runtime_components: &'a RuntimeComponents,
    ...
    /// ```no_run
    /// use aws_sdk_sesv2::config::endpoint::{ResolveEndpoint, EndpointFuture, Params, Endpoint};
    /// #[derive(Debug)]
    ...
    /// let config = aws_sdk_sesv2::Config::builder().endpoint_resolver(resolver).build();
    /// let client = aws_sdk_sesv2::Client::from_conf(config);
    /// ```
    ...
    ///
    /// ```no_run
    /// use aws_sdk_sesv2::config::{AsyncSleep, Config, SharedAsyncSleep, Sleep};
    ///
    /// #[derive(Debug)]
    ...
    ///
    /// ```no_run
    /// use aws_sdk_sesv2::config::{AsyncSleep, Builder, Config, SharedAsyncSleep, Sleep};
    ///
    /// #[derive(Debug)]
    /// pub struct ForeverSleep;
    ///
    ...
    /// # Default Behavior
    ///
    /// When no retry partition is explicitly set, the SDK automatically creates a default retry partition named `sesv2`
    /// (or `sesv2-<region>` if a region is configured).
    /// All SESv2 clients without an explicit retry partition will share this default partition.
    ///
    /// # Notes
    ///
    ...
    /// ```no_run
    /// use aws_sdk_sesv2::config::Config;
    /// use aws_sdk_sesv2::config::retry::{RetryPartition, TokenBucket};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_sesv2::config::Config;
    /// use aws_sdk_sesv2::config::retry::{ClientRateLimiter, RetryConfig, RetryPartition};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_sesv2::config::IdentityCache;
    ///
    /// let config = aws_sdk_sesv2::Config::builder()
    ...
    /// #    fn meta(&self) -> &aws_sdk_sesv2::error::ErrorMetadata { todo!() }
    /// # }
    ///
    ...
    pub fn build(mut self) -> Config {
        ...
        Config {
            config: crate::config::Layer::from(layer.clone())
                .with_name("aws_sdk_sesv2::config::Config")
                .freeze(),
            cloneable: layer,
...
impl ServiceRuntimePlugin {
    pub fn new(_service_config: crate::config::Config) -> Self {
        let config = {
            let mut cfg = ::aws_smithy_types::config_bag::Layer::new("SimpleEmailService_v2");
    ...
    pub(crate) fn new(
        ...
        Self {
            config: ::aws_smithy_types::config_bag::Layer::from(layer)
                .with_name("aws_sdk_sesv2::config::ConfigOverrideRuntimePlugin")
                .freeze(),
            components,
        }
    }
        ...
        if input.get_origin("endpoint_url").is_client_config() {
            ...
            builder.set_endpoint_url(
                input
                    .service_config()
                    .and_then(|conf| {
                        conf.load_config(service_config_key("SESv2", "AWS_ENDPOINT_URL", "endpoint_url"))
...
pub(crate) fn base_client_runtime_plugins(
    ...
    let default_retry_partition = "sesv2";
